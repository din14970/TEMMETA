...
Output: peaks
        ...
        - Gaussian fitting:    [x, y, peak intensity, sigma, x refined, y refined, background from fit, integrated
                                peak intensity]
"""
...
import plotImagePeaks

matplotlib.use("TkAgg")


def fitLatPeaks(*args):
    ...
    image_filt = butterworthFilter.bwfilter(image, 100, 8)
    ...
    # Normalize image
    image_filt = cv2.normalize(image_filt, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    ...
    peaks_or = np.hstack((x_p, y_p, I_p))
    ...
    bound = 5
    del_bound = np.logical_and.reduce((peaks[:, 0] > bound, peaks[:, 0] < Nx-bound, peaks[:, 1] > bound, peaks[:,
                                                                                                          1] < Ny-bound))
    ...
    # Ask for user input to refine fit with non-linear double Gaussian functions
    root: Tk = tk.Tk()
    parent = root
    root.withdraw()
    answer = messagebox.askyesno("Non-linear Gaussian peak fitting?", "Should lattice peaks be fitted with "
    ...
    if answer is True:
        ...
        for p0 in range(0, num_peaks, 1):
            ...
            x_sub = np.arange(np.max((xc-rCut, 0)), np.min((xc+rCut, Nx)) + 2, 1)
        ...
        messagebox.showinfo("Information", "Lattice peaks are fitted without non-linear Gaussian functions!")
